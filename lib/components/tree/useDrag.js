...
const useDrag = ({
  ...
  let initialX = 10;
  let initialY = 10;
  let dragCloneNode = null;
  let sourceTreeItem = null;
  let sourceNode = void 0;
  let hasStyleTreeItem = null;
  const dragLine = document.createElement("div");
  dragLine.classList.add("virt-tree-drag-line");
  dragLine.style.gap = "4px";
  dragLine.style.paddingLeft = `${props.indent + 4}px`;
  const levelArrow = document.createElement("div");
  levelArrow.style.width = "0";
  levelArrow.style.height = "0";
  levelArrow.style.borderLeft = "4px solid #4c88ff";
  levelArrow.style.borderRight = "4px solid transparent";
  levelArrow.style.borderTop = "4px solid transparent";
  levelArrow.style.borderBottom = "4px solid transparent";
  levelArrow.style.position = "absolute";
  levelArrow.style.top = "-3px";
  levelArrow.style.left = "0";
  ...
  function dragstart() {
    ...
    dragCloneNode = sourceTreeItem.cloneNode(true);
    dragCloneNode.classList.add("virt-tree-item--ghost");
    if (props.dragGhostClass) {
      dragCloneNode.classList.add(props.dragGhostClass);
    }
    dragCloneNode.style.position = "fixed";
    dragCloneNode.style.width = `${sourceTreeItemRect.width}px`;
    dragCloneNode.style.height = `${sourceTreeItemRect.height}px`;
    document.body.append(dragCloneNode);
    return dragCloneNode;
  }
  function onMousemove(event) {
    ...
    dragCloneNode.style.left = `${initialX + dx}px`;
    dragCloneNode.style.top = `${initialY + dy}px`;
    const clientElement = (_a = virtListRef.value) == null ? void 0 : _a.$el;
  ...
  function onMouseup() {
    if (dragging.value) {
      ...
      if (dragCloneNode) {
        if (props.dragGhostClass) {
          dragCloneNode.classList.remove(props.dragGhostClass);
        }
        document.body.removeChild(dragCloneNode);
        dragCloneNode = null;
      }
      if (sourceTreeItem) {
        if (props.dragClass) {
